...
with workflow.unsafe.imports_passed_through():
    ...
    from models import ConvertToTextInput, DocumentIngestInput, DocumentIngestResult, DetermineFileTypeInput, SaveConvertedTextInput
...
class DocumentIngestWorkflow:
    ...
    async def run(self, input_data: DocumentIngestInput) -> DocumentIngestResult:
        logger.info("workflow.start", workflow="DocumentIngestWorkflow", file_path=input_data.file_path)
        ...
        result = DocumentIngestResult(
            ...
            mime_type=file_type_result.mime_type,
            text_preview=convert_result.text[:120],
            saved_to=save_result.saved_to,
            record_id=save_result.record_id,
        )
        logger.info("workflow.complete", workflow="DocumentIngestWorkflow", saved_to=result.saved_to)
        return result
